...
def getargs():
    ...
    parser.add_argument('files', nargs='+', help='指定一个或多个文件的路径,也可以是目录')
    parser.add_argument(
        '-e',
        '--encoding',
        nargs='?',
...
def convertEncoding(file, encoding):
    f = open(file, 'r', encoding=encoding,
             errors='replace')
    text = f.read()
    f.close()
    outpath = os.path.join(
        args.outdir, file) if args.outdir else file
    f = open(outpath, 'w', encoding=args.encoding,
             errors='replace')
    f.write(text)
    f.close()
...
if __name__ == '__main__':
    ...
    for file in args.files:
        if not os.path.isfile(file):
            print(f'[-] 无效的文件路径: {file} [-] 可能是目录? 尝试递归扫描')
            for dirRoot, dirs, dirFiles in os.walk(file):
                for fileName in dirFiles:
                    fileFullName = dirRoot + os.sep + fileName
                # continue
                    encoding, confidence = detcect_encoding(fileFullName)
                    print(
                        f'[+] {fileFullName}: 编码 -> {encoding} (置信度 {confidence}%) [+]')
                    if args.encoding and (encoding is not 'unknown') and (confidence > 75):
                        if args.encoding == encoding:
                            print(
                                f'[*] {fileFullName} 已经是 {encoding} 编码了，无需转换！[*]')
                            continue
                    convertEncoding(fileFullName, encoding)
